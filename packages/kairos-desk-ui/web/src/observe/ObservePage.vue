...
import type { ObserveProjection, ProjectProjection, Surface, TraceGroupProjection } from "@/api/types";
...
const selectedIssueEvent = computed(() => selectedTrace.value?.events.find((event) => event.severity === "error" || event.severity === "warning") ?? null);
...
const linkedSummary = computed(() => {
  const evidence = selectedEvidence.value[0];
  ...
  <div class="content-scroll">
    <div class="observe-page">
      <div class="observe-summary-strip" aria-label="Runtime audit summary">
        ...
        <button class="secondary-button" type="button">Replay cursor</button>
      </div>
      ...
      <div class="observe-workspace">
        <aside class="observe-panel trace-index" aria-label="Trace groups">
          ...
          >
            ...
            <span class="trace-group-copy">
              ...
              <span>{{ formatStatus(group.status) }} · {{ group.id }} · {{ group.events.length }} events · {{ group.startedAt }}</span>
            </span>
          </button>
        ...
        <section v-if="selectedTrace" class="observe-audit-stage" aria-label="Selected trace audit">
          ...
          <div class="trace-insight-strip" aria-label="Selected trace summary">
            <div class="trace-insight" :class="selectedIssueEvent ? eventTone(selectedIssueEvent.severity) : ''">
              <span>Issue</span>
              <strong>{{ issueSummary }}</strong>
            </div>
            <div class="trace-insight">
              <span>Latest</span>
          ...
          <div class="observe-audit-grid">
            <section class="observe-panel evidence-board" aria-label="Linked evidence">
              ...
              <button v-for="item in visibleEvidence" :key="item.id" class="evidence-audit-row" type="button" @click="emit('navigate', item.linkedSurface, item.linkedId)">
            ...
            <section class="observe-panel endpoint-board" aria-label="Endpoint health">
              ...
              <div class="endpoint-audit-list">
                <article v-for="endpoint in visibleEndpoints" :key="endpoint.id" class="endpoint-audit-row">
                  <span class="trace-status-dot" :class="statusTone(endpoint.status)" aria-hidden="true" />
                  <div>
                    <strong>{{ endpoint.label }}</strong>
                    <span class="endpoint-audit-meta" :class="statusTone(endpoint.status)">{{ formatStatus(endpoint.status) }} · {{ endpoint.latencyMs }}ms</span>
          ...
          <section class="observe-panel trace-detail" aria-label="Selected trace timeline">
            <div class="audit-section-head">
              <div>
                <div class="kicker">Trace timeline</div>
            ...
            <ol class="observe-event-list">
              <li v-for="event in visibleTraceEvents" :key="event.id" class="observe-event-row">
                <time>{{ event.timestamp }}</time>
                <span class="trace-dot" :class="eventTone(event.severity)" aria-hidden="true" />
                <div>
